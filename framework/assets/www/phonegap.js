 ...
 * @param {Boolean} persist		Whether we should retain the callback after it's been called once.
 */
PhoneGap.exec = function(success, fail, service, action, args, persist) {
    try {
        ...
        if (success || fail) {
            ...
            PhoneGap.callbacks[callbackId] = {success:success, fail:fail, persist: persist};
        }
...
/**
 * Removes a callback. Calls from native code for the command will then be silently ignored.
 *
 * @param {String} callbackId The id of the callback to remove
 */

PhoneGap.removeCallback = function(callbackId) {
	delete PhoneGap.callbacks[callbackId];
}
